...
`useDefaultBootstrap`, or a private request with `repositoryPath`, a 32-byte
base64 `swarmKey`, private `bootstrapPeers`, and optional `relayPeers` /
`allowedPeerIds`.
...
The example is a single-instance, four-page console covering node/public/private
configuration, content and the persistent repository, swarm/bootstrap/
Bitswap/DHT routing, IPNS, capabilities, and typed operation logs. Native
private mode uses the same stack as public mode plus a 32-byte PSK and private
bootstrap list. Web keeps local IndexedDB add/read/pin controls and explicitly
disables private swarms and provider publication. Kubo is only an optional
external acceptance tool and is not part of the Example workflow.
